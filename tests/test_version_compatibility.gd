...
func test_logger_support_detection():
	...
	assert_eq(synclogger_support, is_logger_available,
	         "SyncLoggerのLogger統合サポート検出が正確であること")
...
func test_error_message_capture_based_on_version():
	...
	if logger_support:
		...
		assert_true(_sync_logger.is_capture_errors_enabled(),
		           "Godot 4.5+ではエラーキャプチャがデフォルト有効")

		_sync_logger.set_capture_errors(false)
		assert_false(_sync_logger.is_capture_errors_enabled(),
		            "エラーキャプチャを無効化できること")
	else:
		# Godot 4.0-4.4: エラーキャプチャは無効
		assert_false(_sync_logger.is_capture_errors_enabled(),
		            "Godot 4.0-4.4ではエラーキャプチャが無効")
...
func test_graceful_degradation():
	...
	# 基本機能は影響を受けないこと
	assert_true(_sync_logger.info("test after system calls"),
	           "新機能API呼び出し後も基本機能が動作すること")