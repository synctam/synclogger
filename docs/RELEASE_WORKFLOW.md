...
### 🚨 重要な原則
**releaseブランチへの直接修正は禁止**
- ✅ **正しい**: dev → release（一方向の反映）
- ❌ **禁止**: releaseブランチでの直接編集
- ❌ **禁止**: release → dev（逆流）

すべての変更はdevブランチで行い、ワークフローに従ってreleaseに反映してください。
...
# これは絶対にやってはいけません
git checkout release
echo "fix" >> README.md  # ❌ 直接編集
git commit -m "hotfix"   # ❌ 直接コミット
```
...
# 緊急修正でも必ずdev経由
git checkout release
git cherry-pick <commit>  # ❌ 直接反映
```

❌ **GitHub等での直接編集禁止**
- GitHub Web UI での README.md 直接編集
- その他プラットフォームでの直接ファイル修正
...
# 3. releaseブランチに反映（ワークフロー手順に従う）
git checkout release
git checkout dev -- <修正したファイル>
git commit -m "fix: [dev反映] 修正内容"
```
